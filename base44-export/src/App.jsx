...
import { ArrowRight, ArrowUpRight, Award, Check, ChevronRight, Menu, Moon, Phone, Shield, Sun, Users, X } from 'lucide-react';
import '../../global.css';
    ...
    {open && <div className="lg:hidden fixed inset-0 z-[9999] bg-background flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        <div className="mb-1 pl-6">
          {services.map(s=><a href="#sluzby" onClick={closeMenu} className="block py-2 font-mono text-xs uppercase tracking-widest text-foreground/50 transition-colors hover:text-primary" key={s[0]}>{s[1]}</a>)}
        </div>
...
export default function App() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><Header theme={theme} setTheme={setTheme}/><main>
    ...
    <section id="sluzby" className="ekos-section ekos-section-muted"><div className="ekos-container ekos-stack"><div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between"><div className="ekos-section-header"><p className="ekos-section-label">Naše služby</p><h2 className="ekos-heading-md">Výrobní možnosti</h2></div><a className="ekos-button ekos-button-ghost" href="#sluzby">Všechny služby <ArrowUpRight size={14}/></a></div><div className="ekos-split !items-stretch"><div className="ekos-card relative min-h-[430px] lg:min-h-[530px] flex items-end overflow-hidden"><img className="absolute inset-0 h-full w-full object-cover opacity-50" src="/assets/d5eaf9267_generated_04d3e802.png" alt="Detail výroby EKOS Mimoň"/><div className="absolute inset-0 bg-gradient-to-t from-background via-background/45 to-transparent"/><div className="relative grid gap-5 md:grid-cols-[1fr_.75fr] md:items-end"><h3 className="ekos-heading-md normal-case">Detail výroby,<br/>ne ilustrační stock</h3><p className="ekos-text-body">Pro finální web doporučuji doplnit 6–10 reálných fotografií: svár, konstrukce, montáž, dílna, servisní zásah.</p></div></div><div className="ekos-grid ekos-grid-2">{services.filter(s=>s[0]!=='technologie-cov').sort((a,b)=>a[0]==='elektromontaze'?1:b[0]==='elektromontaze'?-1:0).map(s=>{const item=s[0]==='technologie-cov'?[s[0],'Technologické celky','Návrh, výroba a montáž dílčích technologických sestav pro výrobní a komunální provozy.']:s[0]==='svarovane-konstrukce'?[s[0],s[1],'Rámy, lávky, podpěry, kryty a další ocelové konstrukce podle výkresu nebo provozních požadavků.']:s[0]==='tlakove-nadoby'?[s[0],s[1],'Výroba, úpravy a opravy tlakových zařízení včetně dokumentace, kontroly a servisní podpory.']:s[0]==='vzduchotechnika'?[s[0],s[1],'Dodávky částí vzduchotechnických tras, napojení na technologii a servis provozních systémů.']:s[0]==='opravy-udrzba'?[s[0],s[1],'Servisní zásahy, rekonstrukce, výměny opotřebených částí a úpravy zařízení podle stavu provozu.']:s[0]==='elektromontaze'?[s[0],s[1],'Elektromontážní práce pro výrobní provozy, technologické celky a napojení zařízení včetně přípravy rozvodů.']:[s[0],s[1],'Jednotlivé kusy i menší série: držáky, kryty, přípravky, stojany, zábradlí a technické sestavy.'];return <article className="ekos-card min-h-[150px] lg:min-h-[170px]" key={item[0]}><div><h3 className="font-bold text-lg uppercase mb-2">{item[1]}</h3><p className="text-muted-foreground text-sm leading-relaxed">{item[2]}</p></div></article>})}</div></div><div className="ekos-grid ekos-grid-4 gap-px bg-border overflow-hidden">{[['Rozsah','návrh, výroba, montáž, servis'],['Podklady','výkres, zaměření nebo technické zadání'],['Materiály','ocel, nerez, hliník dle požadavku'],['Výstup','dokumentace a předání zakázky']].map(x=><div className="bg-background p-6 lg:p-7 group hover:bg-secondary transition" key={x[0]}><span className="ekos-stat-label normal-case">{x[0]}</span><p className="font-bold text-base uppercase leading-snug mt-4">{x[1]}</p></div>)}</div></div></section>
