...
function ServiceCard({ item, className = '' }) { return <article className={`ekos-card ${className}`}><div><h3 className="font-bold text-lg uppercase mb-2">{item[1]}</h3><p className="text-muted-foreground text-sm leading-relaxed">{item[2]}</p></div></article>; }
...
export default function App() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><Header theme={theme} setTheme={setTheme}/><main>
    ...
    <section id="sluzby" className="ekos-section ekos-section-muted"><div className="ekos-container ekos-stack"><div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between"><div className="ekos-section-header"><p className="ekos-section-label">Naše služby</p><h2 className="ekos-heading-md">Výrobní možnosti</h2></div><a className="ekos-button ekos-button-ghost" href="#sluzby">Všechny služby <ArrowUpRight size={14}/></a></div><div className="grid gap-6 lg:grid-cols-[minmax(0,2.05fr)_minmax(280px,.95fr)] lg:items-stretch"><div className="ekos-card relative min-h-[440px] lg:min-h-[620px] flex items-end overflow-hidden"><img className="absolute inset-0 h-full w-full object-cover opacity-55" src="/assets/d5eaf9267_generated_04d3e802.png" alt="Detail výroby EKOS Mimoň"/><div className="absolute inset-0 bg-gradient-to-t from-background via-background/30 to-transparent"/><div className="relative grid gap-5 md:grid-cols-[1fr_.75fr] md:items-end"><h3 className="ekos-heading-md normal-case">Detail výroby,<br/>ne ilustrační stock</h3><p className="ekos-text-body">Pro finální web doporučuji doplnit 6–10 reálných fotografií: svár, konstrukce, montáž, dílna, servisní zásah.</p></div></div><div className="grid gap-6">{serviceCards.slice(0,2).map(item=><ServiceCard key={item[0]} item={item} className="min-h-[190px] lg:min-h-0"/>)}</div></div><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">{serviceCards.slice(2).map(item=><ServiceCard key={item[0]} item={item} className="min-h-[160px]"/>)}</div><div className="ekos-grid ekos-grid-4 gap-px bg-border overflow-hidden">{[['Rozsah','návrh, výroba, montáž, servis'],['Podklady','výkres, zaměření nebo technické zadání'],['Materiály','ocel, nerez, hliník dle požadavku'],['Výstup','dokumentace a předání zakázky']].map(x=><div className="bg-background p-6 lg:p-7 group hover:bg-secondary transition" key={x[0]}><span className="ekos-stat-label normal-case">{x[0]}</span><p className="font-bold text-base uppercase leading-snug mt-4">{x[1]}</p></div>)}</div></div></section>
